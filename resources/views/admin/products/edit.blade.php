...
<head>
    ...
    <title>Edit Varian Rasa - Brasil Es Krim & Es Puter</title>
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <header class="bg-white border-b border-slate-200 px-8 py-4 flex justify-between items-center shadow-sm">
                <div class="flex items-center space-x-2 text-slate-500 text-sm">
                    ...
                    <span>/</span>
                    <span class="font-medium text-slate-800">Edit Varian</span>
                </div>
            ...
            <main class="flex-1 overflow-y-auto p-8">
                <div class="max-w-3xl mx-auto bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <div class="flex justify-between items-center mb-6 pb-4 border-b border-slate-100">
                        <div>
                            <h2 class="text-xl font-bold text-slate-800">Edit Varian Rasa</h2>
                            <p class="text-sm text-slate-500">Ubah detail data produk rasa {{ $product->name }}.</p>
                        </div>
                    ...
                    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="w-full border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-red-500 focus:outline-none transition">
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="category" class="w-full border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-red-500 focus:outline-none transition">
                                    <option value="Es Krim" {{ $product->category == 'Es Krim' ? 'selected' : '' }}>Es Krim</option>
                                    <option value="Es Puter" {{ $product->category == 'Es Puter' ? 'selected' : '' }}>Es Puter</option>
                                    <option value="Es Mambo / Lilin" {{ $product->category == 'Es Mambo / Lilin' ? 'selected' : '' }}>Es Mambo / Lilin</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="status" class="w-full border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-red-500 focus:outline-none transition">
                                    ...
                                    <option value="sold_out" {{ $product->status == 'sold_out' ? 'selected' : '' }}>Habis</option>
                                </select>
                            </div>
                        </div>

                        <div>
                            <label class="block font-semibold text-slate-700 mb-2">Foto Kemasan / Display</label>
                            @if($product->image)
                                <div class="mb-3">
                                    <img src="{{ asset('storage/' . $product->image) }}" class="w-32 h-32 object-cover rounded-xl border border-slate-200">
                                </div>
                            @endif
